I apologize for that formatting slip-up. Here is the entire response structured correctly in Markdown, including the code blocks and technical sections, so you can copy and paste it directly into a .md file or documentation.
 ...
 * $new_role: The string slug of the new role (e.g., 'subscriber').
...
function sync_africaneur_status_to_mailchimp($user_id, $new_role, $old_roles) {
    ...
    // These variables connect your WordPress site to your specific Mailchimp account.
    ...
    // This array acts as a translation table. 
    ...
    // If not, we stop the function here to avoid unnecessary API calls.
    ...
    // We set the status of the NEW role to 'active'.
    // We set all other roles in the map to 'inactive' to ensure tags are swapped, not stacked.
    $tags_to_send = [];
    ...
    // We build the URL for the Mailchimp "Tags" endpoint.
    $url = "https://{$server_prefix}.api.mailchimp.com/3.0/lists/{$audience_id}/members/{$subscriber_hash}/tags";

    // wp_remote_post is the WordPress way of sending an HTTP request.
    $response = wp_remote_post($url, [
        'method'  => 'POST',
        'headers' => [
            // Mailchimp requires Basic Auth (any string as username + the API key).
            'Authorization' => 'Basic ' . base64_encode('user:' . $api_key),
            'Content-Type'  => 'application/json',
        ],
        // The body contains our array of tags converted to a JSON string.
        'body'    => json_encode(['tags' => $tags_to_send]),
    ]);

    // --- SECTION 6: ERROR LOGGING ---
    // If the request fails (e.g., site is down or timeout), it logs the error to wp-content/debug.log.
